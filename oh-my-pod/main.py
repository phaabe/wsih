    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            if key in ["Cover", "id", "createdTime"]:
                ...
                cleaned_data[key] = [
                    remove_image_data(item) if isinstance(item, dict) else item
                    for item in value
                ]
            else:
                cleaned_data[key] = value
        ...
        return [
            remove_image_data(item) if isinstance(item, dict) else item for item in data
        ]
    return data


def download_cover_image(
    cover_url: str, podcast_name: str, cover_dir: str = "cover"
) -> str:
    ...
    try:
        ...
        safe_name = "".join(
            c for c in podcast_name if c.isalnum() or c in (" ", "-", "_")
        ).rstrip()
                ...
                if "fields" in record and "Cover" in record["fields"]:
                    ...
                    if isinstance(cover_data, list) and len(cover_data) > 0:
                        ...
                        if cover_image and "url" in cover_image:
                            ...
                            podcast_name = record["fields"].get(
                                "Name", f"podcast_{record.get('id', 'unknown')}"
                            )

                            # Download cover and get local path
                            local_path = download_cover_image(
                                cover_image["url"], podcast_name
                            )
...
if __name__ == "__main__":
    ...
    if not os.environ.get("BEARER_TOKEN"):
        print(
            "Warning: BEARER_TOKEN environment variable not set. Request will fail.",
            file=sys.stderr,
        )
